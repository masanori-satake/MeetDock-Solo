...
### 修正 (Fixed)
- `.ics` インポート時の重複判定に安定したオカレンス識別子を使用し、繰り返し会議の開始時刻が進んだ後の再インポートを防ぎつつ、`RDATE` と `RECURRENCE-ID` の各回を個別に保持するよう修正
- `.ics` ファイル選択コマンドとダイアログの表示を日本語・英語にローカライズ
- `text/x-vcalendar` のみを含むドロップでも `.ics` 会議を登録できるよう修正
