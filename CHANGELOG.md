...
### 修正 (Fixed)
- `visibility: "collapsed"` は使い方ガイドビューの初回表示時の既定値であり、既存ワークスペースの表示状態は変更しないことが明確になるよう、バージョン 1.12.1 の変更内容を訂正

## [1.12.1] - 2026-09-14

### 修正 (Fixed)
- サイドバー下部の「使い方ガイド」ビューを常時利用可能にし、初回表示時は折りたたまれる構成に修正
  - 「使い方ガイド」ビュー（`meetdock-guide-view`）の `when` 条件を解除し、完全非表示にならないよう変更
  - `visibility: "collapsed"` を初回表示時の既定値として設定（既存ワークスペースに保存済みの表示状態は維持）
